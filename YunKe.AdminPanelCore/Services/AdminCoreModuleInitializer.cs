...
namespace YunKe.AdminPanelCore.Services
{
    /// <summary>
    /// 模块初始化
    /// </summary>
    public class AdminCoreModuleInitializer : ModuleInitializer
    {
        /// <summary>
        /// 加载SimpleInject配置
        /// </summary>
        ...
        {
            var registrations =
                from type in typeof (AdminCoreModuleInitializer).Assembly.GetTypes()
                where
                    type.Namespace != null && (type.Namespace.IsNotBlank() &&
                                               type.Namespace.StartsWith("YunKe.AdminPanelCore.Services.AppServices") &&
        ...
        public override void LoadAutoMapper(IMapperConfiguration config)
        {
            ...
            config.CreateMap<FriendLink, FriendLinkDto>().ReverseMap();
        }
    }
}
