...
namespace YunKe.AdminPanel
{
    ...
    public class SiteExceptionAttribute : HandleErrorAttribute
    {
        ...
        private void ProcessException(ExceptionContext exceptionContext)
        {
            ...
            {
                var dve = exception as DbEntityValidationException;

                if (dve.EntityValidationErrors != null)
                {
                    foreach (var item in dve.EntityValidationErrors.SelectMany(x => x.ValidationErrors))
                    {
                        content += "<p><b>" + item.PropertyName + "</b>" + item.ErrorMessage + "</p>";
                    }
